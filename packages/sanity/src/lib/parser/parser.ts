...
import type { CstGram, CstLabels, CstNode, CstRecord, CstProperty, CstRelationship, GramElement, CstSyntaxTypeTag } from '../ast/ast-elements';
...
export interface GramSyntaxNode extends Parser.SyntaxNode, CstGram {
...
export const reduce = <T>(cst:GramElement, f:(cst:GramElement, acc:T) => T, acc:T):T => {
  const result = (cst.children?.length > 0) ?
...
export const map = <T extends Record<string, unknown>>(cst:GramElement, f:(cst:GramElement) => T):Treelike<T> => ({
  ...f(cst),
...
export const isCstSyntax = (o:unknown):o is GramElement => (typeof o === "object") && (o !== null) && ("type" in o);
