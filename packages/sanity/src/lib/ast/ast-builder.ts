import { GramIdentifier, GramSymbol, GramNode, GramPattern, GramRelationship, GramAnnotation, GramRelation } from './ast-elements'
import { BooleanLiteral, DateLiteral, DecimalLiteral, DurationLiteral, GramPropertyRelation, GramPropertyValue, GramRecord, GramRecordEntry, HexadecimalLiteral, IntegerLiteral, MeasurementLiteral, OctalLiteral, StringLiteral, TaggedTextLiteral, TimeLiteral, isGramLiteral } from './ast-record'

const dateToYMD = (d: Date) => d.toISOString().slice(0, 10);

const dateToDayOfMonth = (d: Date) => '--' + d.toISOString().slice(5, 10);

export const recordValue = (relation: GramPropertyRelation, value: GramPropertyValue): GramRecordEntry => ({
  relation,
  value
})
...
export const pattern = (members:ReadonlyArray<GramPattern>, identifier?:string, labels?:string[], record?:GramRecord, relation?:GramRelation):GramPattern => ({
...
export const annotation = (subject:GramPattern, identifier?:string, labels?:string[], record?:GramRecord):GramAnnotation =>
  pattern([subject], identifier, labels, record) as GramAnnotation;

export const node = (identifier?:string, labels?:string[], record?:GramRecord):GramNode => 
  pattern([], identifier, labels, record) as GramNode;

export const relationship = (from:GramNode, relation:GramRelation, to:GramNode, identifier?:string, labels?:string[], record?:GramRecord):GramRelationship => 
  pattern([from,to], identifier, labels, record, relation) as GramRelationship;
