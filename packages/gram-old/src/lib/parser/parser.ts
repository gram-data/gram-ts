...
export const reduce = <T>(
  cst: GramSyntaxNode,
  f: (cst: GramSyntaxNode, acc: T) => T,
  acc: T
): T => {
  const result =
    cst.children !== null && cst.children.length > 0
...
export const stats = (cst: GramSyntaxNode) =>
  reduce(
    cst,
    (cst, acc) => {
      return pipe(
...
export const nodes = (cst: GramSyntaxNode): CstNode[] =>
  reduce(
    cst,
    (cst, acc) => {
...
export const relationships = (cst: GramSyntaxNode): CstRelationship[] =>
  reduce(
    cst,
    (cst, acc) => {
...
export const labels = (cst: GramSyntaxNode): Set<string> =>
  reduce(
    cst,
    (cst, acc) => {
      if (isCstLabels(cst))
...
export const properties = (cst: GramSyntaxNode): CstProperty[] =>
  reduce(
    cst,
    (cst, acc) => {
