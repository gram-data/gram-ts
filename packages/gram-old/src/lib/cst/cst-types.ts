...
) => CstNode = S.decodeUnknownSync(CstNode);
...
export const CstRelationshipValueUnion = S.Union(
  CstSingleRelationship.pipe(
    S.attachPropertySignature('type', 'single_undirected')
  ),
  CstDoubleRelationship.pipe(
    S.attachPropertySignature('type', 'double_undirected')
  ),
  CstSquiggleRelationship.pipe(
    S.attachPropertySignature('type', 'squiggle_undirected')
  )
);
