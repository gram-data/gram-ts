...
describe('Subjects', () => {
  ...
  it('should compare two subjects by value equality', () => {
    const personA1 = Subject.of({
      identifier: 'a',
      labels: ['Person'],
      record: {},
    });
    const personA2 = Subject.of({
      identifier: 'a',
      labels: ['Person'],
      record: {},
    });
    expect(Subject.equalSignature(personA1, personA2)).toBe(true);
  });
  ...
  it('should construct a subject from a named object', () => {
    const s = Subject.fromNamedObject("o", { name: 'ABK', height: 185 });
    
    expect(s.record.name).toEqual('ABK');
    expect(s.record.height).toEqual(185);
    expect(s.identifier).toEqual("o");
    expect(s.labels.length).toBe(0);
  });
