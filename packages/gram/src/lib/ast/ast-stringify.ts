...
export const stringifyPatternExpression = (pattern: GramPattern): string => {
  const attrs = stringifyAttributes(pattern);
  const members = pattern.members.map(stringify).join(",");
  return `[${attrs}${attrs.length > 0 ? ' ' : ''}${members}]`
}
...
export const stringify = (pattern: GramPattern): string => {
  ...
  } else if (pattern.members.length > 0) {
    return stringifyPatternExpression(pattern)
  } else {
