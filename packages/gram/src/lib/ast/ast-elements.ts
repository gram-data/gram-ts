...
export const isGramNode = (pattern: GramPattern): pattern is GramNode =>
 ...
 * which are both nodes, and with a required relation.
 */
export interface GramRelationship extends GramPattern {
  readonly relation: GramRelation
  readonly members: [GramNode, GramNode];
}

export const isGramRelationship = (
  pattern: GramPattern
): pattern is GramRelationship =>
  (pattern.relation !== undefined) && (pattern.relation !== null) && pattern.members.length === 2 && pattern.members.every(isGramNode);
